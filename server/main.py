...
def get_purchase_order(backlog_item_id: str):
    """Get the purchase order created for a backlog item"""
...
def create_purchase_order(request: CreatePurchaseOrderRequest):
    ...
    if any(po["backlog_item_id"] == request.backlog_item_id for po in purchase_orders):
        raise HTTPException(status_code=400, detail=f"A purchase order already exists for backlog item {request.backlog_item_id}")
...
def create_task(request: CreateTaskRequest):
    """Create a new task"""
    # max()-based ids stay unique after deletions, unlike len()+1
